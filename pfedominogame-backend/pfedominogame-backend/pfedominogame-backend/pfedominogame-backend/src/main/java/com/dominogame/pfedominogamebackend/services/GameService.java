...
public class GameService {
    ...
    @Autowired
    public GameService(GameRepository gameRepository, PlayerRepository playerRepository, DominoRepository dominoRepository, MoveRepository moveRepository, PlayerService playerService) {
   ...
   @Transactional
   public Game initialGame(@PathVariable Long gameId) {
       // Récupérer le jeu à partir de l'ID
       Game game = gameRepository.findById(gameId)
               .orElseThrow(() -> new IllegalArgumentException("Game not found with ID: " + gameId));
    ...
    public Game gamePlay(Long gameId, Long playerId, int dominoIndex, char side,char skip) {
        ...
        setofPlayer = (setofPlayer == setofPlayer1)?setofPlayer2:setofPlayer1;
